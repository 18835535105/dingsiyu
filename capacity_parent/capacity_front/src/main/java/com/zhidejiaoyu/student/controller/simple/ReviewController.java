...
import com.zhidejiaoyu.student.service.simple.ReviewService;
...
public class ReviewController {
    ...
    public ServerResponse<Object> testCapacityReview(String unit_id, int classify, HttpSession session) throws JsonProcessingException {
        if(classify == 0){
            // 单词图鉴
            return reviewService.testReviewWordPic(unit_id, classify, session);
        }else {
            return reviewService.testCapacityReview(unit_id, classify, session);
        }
    ...
    public ServerResponse<Map<String, Object>> CapacityReview(String unitId, String course_id, int classify, String judge, HttpSession session) throws Exception {
        ...
        // 图片图鉴模块
        if(classify == 0){
            return reviewService.Reviewcapacity_picture(id, unitId, 0, course_id, judge);
        }
        // 慧记忆模块 / 听写 / 默写
        if (classify == 1 || classify == 2 || classify == 3) {
            // 从记忆追踪-...中查询一条记录
            Map<String, Object> map = reviewService.ReviewCapacity_memory(id, unitId, classify, course_id);
            return ServerResponse.createBySuccess(map);
        }
        // 例句听写
        if (classify == 4 || classify == 5 || classify == 6) {
            Map<String, Object> map = reviewService.ReviewSentence_listen(id, unitId, classify, course_id);
            return ServerResponse.createBySuccess(map);
        }
    ...
    public ServerResponse<Object> testcentre(String courseId, String unitId, int select, int classify, Boolean isTrue, HttpSession session) {
        ...
        if (classify == 1 || classify == 2 || classify == 3) {
            ...
            // 例句题,
            return ServerResponse.createBySuccess(reviewService.testcentreSentence(unitId, select, classify, session));
        }
    }
