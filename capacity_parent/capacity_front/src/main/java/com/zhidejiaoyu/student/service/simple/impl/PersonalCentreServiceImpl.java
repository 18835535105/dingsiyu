...
import com.zhidejiaoyu.student.service.simple.PersonalCentreService;
...
public class PersonalCentreServiceImpl extends BaseServiceImpl<CcieMapper, Ccie> implements PersonalCentreService {
    ...
    private void getWorshipRanking(String model, String worship, Student student, Map<String, Object> map, Integer start, Integer rows) {
        ...
        Map<Long, Map<String, Object>> longMapMap1 = awardMapper.selCountAwardByStudentIds(studentIds);
    ...
    public ServerResponse<Object> showCcie(HttpSession session, Integer model) {
        ...
        for (Map<String, Object> map : listCcie) {
            ...
            if (tempMap.get("testName") == null) {
                ...
                if (model == 1) {
                    myCcieVo.setTestName(commonMethod.getCapacityStudyModel(Integer.valueOf(map.get("model").toString())));
                } else {
    ...
    public ServerResponse<List<Map<String, Object>>> getMedalInClass(HttpSession session) {
        ...
        Date today = new Date();
        List<Map<String, String>> medalMap = awardMapper.selectLatestMedalInClass(student, today);
        if (medalMap.size() == 0) {
            // 如果当前没有查询出已领取的记录，查找昨天的领取记录
            Date yesterday = new DateTime().plusDays(-1).toDate();
            medalMap = awardMapper.selectLatestMedalInClass(student, yesterday);
        }
        if (medalMap.size() > 0) {
