...
public class SimplePersonalCentreServiceImplSimple extends SimpleBaseServiceImpl<SimpleCcieMapper, Ccie> implements SimplePersonalCentreServiceSimple {
    ...
    public ServerResponse<Object> weekDurationIndex(HttpSession session) {
        ...
        for (Integer year : years) {
            ...
            for (int i = 0; i <= yearMax; i++) {
                ...
                // 查询循环周的总有效时长(m)
                Integer duration = simpleDurationMapper.totalTime(DateUtil.formatYYYYMMDD(weekStart), DateUtil.formatYYYYMMDD(weekEnd), studentId);
    ...
    public ServerResponse<Object> weekDurationIndexPageTwo(HttpSession session, int page, int rows, Integer yea) {
        ...
        if (student.getFirstStudyTime() != null) {
            //总周数
            int timeGapDays = WeekUtil.getTimeGapDays(student.getFirstStudyTime());
            //开始查找的位置
            ...
            for (int i = start; i < end; i++) {
                if (i <= timeGapDays + 1) {
                    ...
                    Date beforWeekDate = DateUtil.getBeforeWeekDate(new Date(), i);
                    Date onMonday = WeekUtil.getMondayDate(beforWeekDate, 1);
                    Date onSunday = WeekUtil.getMondayDate(beforWeekDate, 7);
                    ...
                    map.put("state", false); // 不是本周
                    // 查询循环周的总有效时长(m)
                    Integer duration = simpleDurationMapper.totalTime(DateUtil.formatYYYYMMDD(onMonday), DateUtil.formatYYYYMMDD(onSunday), student.getId());
                    String timeStrBySecond = TimeUtil.getTimeStrBySecond(duration);
                    int plan = ((int) (BigDecimalUtil.div((duration == null ? 0 : duration), 12600) * 100));
                    if (plan <= 100) {
                        map.put("pluSign", false);// 不显示+号
        ...
        if (student.getFirstStudyTime() != null) {
            ...
            for (int i = start; i < end; i++) {
                if (i <= timeGapDays + 1) {
                    ...
                    beforeWeekDate = DateUtil.getBeforeWeekDate(new Date(), i);
                    onMonday = WeekUtil.getMondayDate(beforeWeekDate, 1);

                    onSunday = WeekUtil.getMondayDate(beforeWeekDate, 7);
                    // 每周开始日期
