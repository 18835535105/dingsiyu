...
import com.zhidejiaoyu.common.study.simple.WordPictureUtil;
...
//            map.put("memoryDifficulty", hard);
    ...
    public Map<String, Object> ReviewSentence_listen(Long stuId, String unit_id, int classify, String course_id) {
        ...
        if (classify != 5) {
            // 例句英文乱序集合
            map.put("orderEnglish", commonMethod.getOrderEnglishList(sentence.getCentreExample(), sentence.getExampleDisturb()));
            // 例句英文正序集合，含有标点
            map.put("englishList", commonMethod.getEnglishList(sentence.getCentreExample()));
        }

        if (classify == 5) {
            // 例句翻译中含有乱序的中文解释和干扰项
            map.put("orderChinese", commonMethod.getOrderChineseList(sentence.getCentreTranslate(), sentence.getTranslateDisturb()));
        }
