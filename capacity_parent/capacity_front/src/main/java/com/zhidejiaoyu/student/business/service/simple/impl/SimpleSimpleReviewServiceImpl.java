...
import com.zhidejiaoyu.student.business.service.simple.SimpleReviewService;
...
public class SimpleSimpleReviewServiceImpl implements SimpleReviewService {
    ...
    public ServerResponse<Object> testCapacityReview(String unit_id, int classify, HttpSession session) {
        ...
        if (classify == 1 || classify == 2 || classify == 3 || classify == 4 || classify == 5 || classify == 6) {
            ...
            // 处理结果
            List<SimpleTestResultVO> testResults = simpleTestResultUtil.getWordTestesForUnit(type, vocabularies.size(), vocabularies, Long.valueOf(unit_id));
    ...
    public Map<String, Object> ReviewCapacity_memory(Long id, String unit_id, int classify, String course_id) throws Exception {
        ...
        if (StringUtils.isNotBlank(course_id)) {
            ...
            map.put("plan", count_); // ./
            // 该课程一共多少单词
            Integer count = unitMapper.countWordByCourse(Long.parseLong(course_id));
            map.put("wordCount", count); // /.
        }
        // 查询条件3:模块
        ca.setClassify(classify + "");
...
//            map.put("memoryDifficulty", hard);
    ...
    public ServerResponse<Object> testcentre(String courseId, String unitId, int select, int classify, Boolean isTrue, HttpSession session) {
        ...
        if (isTrue) {
            ...
            Integer judgeTest = simpleTestRecordMapper.selectJudgeTestToModel(courseId, studentId, classify, select);
            if (judgeTest != null) {
                // 已经测试过, 提示扣除金币是否测试
        ...
        if (classify == 1) {
            ...
            // 处理结果
            List<SimpleTestResultVO> testResults = simpleTestResultUtil.getWordTestesForCourse(type, vocabularies.size(), vocabularies, Long.valueOf(courseId));
            ...
            // 处理结果添加单元id
            for (SimpleTestResultVO reList : testResults) {
                // 单词id
    ...
    public Map<String, Object> testcentreSentence(String unitId, int select, int classify, HttpSession session) {
        ...
        if (select == 1) {
            // 查询 例句,例句翻译
            vo = capacityMapper.centreReviewSentence_listen(studentId, unitId, classifyStr);
            // 生词
        } else if (select == 2) {
            vo = capacityMapper.accrueCentreReviewSentence_listen(studentId, unitId, classifyStr);
            // 熟词
        } else if (select == 3) {
            vo = capacityMapper.ripeCentreReviewSentence_listen(studentId, unitId, classifyStr);
        }
        // 1.例句读音
    ...
    public ServerResponse<Object> fiveDimensionTest(String courseId, boolean isTrue, HttpSession session) {
        ...
        if (isTrue) {
            ...
            Integer judgeTest = simpleTestRecordMapper.selectJudgeTest(courseId, studentId, "单词五维测试");
            if (judgeTest != null) {
                // 已经测试过, 提示扣除金币是否测试
                return ServerResponse.createBySuccess(GoldResponseCode.NEED_REDUCE_GOLD.getCode(), "您已参加过该五维测试，再次测试需扣除1金币。");
            }
        ...
        // 查询课程下边一共有多少单词
        Integer countWord = unitMapper.countWordByCourse(Long.parseLong(courseId));
        if (countWord == 0) {
        ...
        String[] type_b = {"汉译英"};
        String[] type_c = {"听力理解"};
        List<SimpleTestResultVO> testResults_a = simpleTestResultUtil.getWordTestesForCourse(type_a, vocabularies.subList(0, vocabularies.size() / 3).size(),
                vocabularies.subList(0, vocabularies.size() / 3), Long.valueOf(courseId));
        List<SimpleTestResultVO> testResults_b = simpleTestResultUtil.getWordTestesForCourse(type_b, vocabularies.subList(vocabularies.size() / 3, (int) BigDecimalUtil.div((double) vocabularies.size(), 1.5, 0)).size(),
                vocabularies.subList(vocabularies.size() / 3, (int) BigDecimalUtil.div((double) vocabularies.size(), 1.5, 0)), Long.valueOf(courseId));
        List<SimpleTestResultVO> testResults_c = simpleTestResultUtil.getWordTestesForCourse(type_c, vocabularies.subList((int) BigDecimalUtil.div((double) vocabularies.size(), 1.5, 0), vocabularies.size()).size(),
     ...
     */
    @Override
    public ServerResponse<Map<String, Object>> Reviewcapacity_picture(Long studentId, String unitId, int model, String courseId, String judge) {
        ...
        if (judge != null && StringUtils.isNotEmpty(unitId)) {
            // 根据单元查询
            correct = simpleCapacityPictureMapper.selectNeedReviewWord(Long.valueOf(unitId), studentId, DateUtil.DateTime());
        } else {
            // 根据课程查询 课程复习模块
            correct = simpleCapacityPictureMapper.selectNeedReviewWordCourse(courseId, studentId, DateUtil.DateTime());
