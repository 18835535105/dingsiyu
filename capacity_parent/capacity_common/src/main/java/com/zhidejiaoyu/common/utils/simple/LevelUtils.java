...
public class LevelUtils {
    ...
    private static Double getAward(Long studentId,List<Map<String, Object>> maps) {
        Double isDouble = 0.0;
        for (Map<String, Object> map : maps) {
            String name = (String) map.get("name");
            if (name.equals("LV1")) {
                Long number = (Long) map.get("number");
                isDouble += number * 0.001;
            }
            if (name.equals("LV2")) {
                Long number = (Long) map.get("number");
                isDouble += number * 0.005;
            }
            if (name.equals("LV3")) {
                Long number = (Long) map.get("number");
     ...
     * @return
     */
    public static Map<String, Double> getAddition(Integer challLevel, Integer beChallLevel, Integer type, Long studentId, Map<String, Double> map,List<Map<String, Object>> maps) {
        ...
        if (beChallLevel - challLevel > 0) {
            if (type == 1) {
                map.put("level", getProportion(level));
            } else {
                map.put("level", getDraw(level));
            }
        } else {
            map.put("level", 0.0);
        }
        map.put("award", getAward(studentId,maps));
