 ...
 * @since 2018-11-19
 */
public interface ExhumationMapper extends BaseMapper<Exhumation> {
     ...
     * @return
     */
    @Select("select final_name finalName, count(id) count , name name from exhumation where student_id=#{studentId} and type!=3 and (state = 1 or state =2) group by final_name ")
    ...
    @Update("update exhumation set state =1 where student_id=#{studentId} and final_name=#{name} and state = 1")
